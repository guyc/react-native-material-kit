...
class Thumb extends Component {
  ...
  _getOnSliding() {
    ...
    return ({ value }) => {
      ...
      const isCollapsed = this.props.collapsed || this.props.collapsed === undefined && x === 0;
      if (isCollapsed) {
        this._onCollapse();
      } else {
        this._onExplode();
...
Thumb.propTypes = {
  ...
  // If true, render as collapsed (unfilled).
  // If false, render as exploded (filled).
  // If undefined, fill as collapsed only if at x=0
